...
export function getSubdir(memoryDir: string, type: string): string {
  return path.join(memoryDir, `${type}s`);
}

export function getNodeFilePath(memoryDir: string, id: string): string {
  // id format: "type/slug" → "types/slug.md"
  const [type, slug] = id.split('/');
  return path.join(memoryDir, `${type}s`, `${slug}.md`);
}

export function parseNodeId(filePath: string): string | null {
...
export function initMemoryDir(projectRoot: string): string {
...
export function listNodeFiles(memoryDir: string): string[] {
  const files: string[] = [];
  for (const subdir of SUBDIRS) {
...
export function getFilePath(memoryDir: string, filename: string): string {
